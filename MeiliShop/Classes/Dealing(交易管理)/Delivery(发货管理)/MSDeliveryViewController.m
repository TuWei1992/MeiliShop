...
@interface MSDeliveryViewController () <UITableViewDataSource, UITableViewDelegate>
...
static NSString *DeliverOrderListOperateCellID = @"DeliverOrderListOperateCell";
static NSString *OrderListInfoTipCellID = @"OrderListInfoTipCell";

@implementation MSDeliveryViewController
...
- (void)setUpChildVc {
    [self addOneChildVc:[[UITableViewController alloc] init] title:@"等待发货"];
    [self addOneChildVc:[[UITableViewController alloc] init] title:@"已发货"];
}
...
- (void)setUpCellInfo {
    ...
    MSCellInfoModel *cellInfoDeliverOperate = [MSCellInfoModel cellInfoWithNibName:@"MSDeliverOrderListOperateCell" reuseIdentifier:DeliverOrderListOperateCellID height:45];
    ...
    self.cellInfos = @[cellInfoDesc, cellInfoDetail, cellInfoFooter, cellInfoDeliverOperate];
...
- (void)loadNewOrder {
    ...
    [self.mgr GET:@"http://shop-mobapi.bizfe.meilishuo.com/order/deliver_list" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[MSOrderListInfoTipCell class]]) {
        ...
        switch (self.selectedIndex) {
            case 1:
                tipCell.refund_status = @"发货状态:已发货";
                break;
            default:
                break;
        }
